...
@click.option('--n-points', '-no', default=50, help="Number of operating points (Mf, Nrot)")
@click.option('--n-inlet', '-ni', default=10, help="Number of inlet conditions (Pin, Tin)")
def main(geometries, output, output_row_size, fluid_list, fluid_type, batch_size, n_points, n_inlet):
    ...
    output_subfolder = pathlib.Path(output) / (os.path.splitext(geometries.parts[-1])[0] + "_map_tabular")
    output_subfolder.mkdir(exist_ok=True)

    geom_ds = ds.dataset(geometries)

    lb_mn = np.array([parameters['m_in'][0], parameters['mach_tip'][0]])
    ub_mn = np.array([parameters['m_in'][1], parameters['mach_tip'][1]])

    xx, yy = np.meshgrid(np.linspace(0, 1, num=n_points, endpoint=True),
                         np.linspace(0, 1, num=n_points, endpoint=True))
    grid = np.c_[xx.ravel(), yy.ravel()]
    X_nm = grid * (ub_mn - lb_mn) + lb_mn
    ...
    for b in geom_ds.to_batches(batch_size=batch_size, columns=['geom_id']):
        ...
        m_in = np.tile(X_nm[:, 0], n_geom*n_inlet)
        mach_tip = np.tile(X_nm[:, 1], n_geom*n_inlet)

        df = pd.DataFrame({
            'geom_id': geom_idx.repeat(n_inlet*n_points**2),
            'fluid': fluids.repeat(n_points**2),
            'in_T': Teff.repeat(n_points**2),
            'in_P': Peff.repeat(n_points**2),
            'in_m_in0': m_in,
