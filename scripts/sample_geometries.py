...
def eval_(node, iparams):
    if isinstance(node, ast.Num):  # <number>
        ...
        return operators[type(node.op)](
            eval_(node.left, iparams), eval_(node.right, iparams)
        )
    elif (
        isinstance(node, ast.Compare) and len(node.ops) == 1
    ):  # <left> <operator> <right> only single comparison supported
        return operators[type(node.ops[0])](
            eval_(node.left, iparams), eval_(node.comparators[0], iparams)
        )
