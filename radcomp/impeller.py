...
class Impeller:
    ...
    def __post_init__(self, geom: Geometry, op: OperatingCondition, ind: Inducer) -> None:
    ...
    def skin_friction_losses(self, geom: Geometry, w4: float, tp4: ThermoProp) -> float:
        ...
        Re = Dh * (self.in2.w + w4)/2 * (self.in2.static.D + tp4.D)/2 / ((self.in2.static.V + tp4.V)/2)
        Cf = moody(Re, geom.rug_imp/Dh)
        return 4*Cf*Lh*((self.in2.w + w4)/2)**2 / (2*Dh)

    def diffusion_factor(self, geom: Geometry, out_H: float, w4: float, n_rot: float) -> float:
        _, Lh = geom.hydraulic_diameter
        wx = self.in2.w
        dh_aero = (out_H)/(n_rot*geom.r4)**2
        # Coppage and Rodgers
        Df = (1 - w4/wx
              + pi*geom.r4**2*dh_aero*n_rot/((geom.n_blades+geom.n_splits)*Lh*wx)
              + 0.1*(geom.r2s-geom.r2h+geom.b4)/2/(geom.r4-geom.r2s)*(1 + w4/wx))
        return Df
    ...
    def clearance_losses(self, geom: Geometry, tp4: ThermoProp, c4t: float, n_rot: float) -> float:
        """Calculates the impeller tip clearance losses according to Jansen, Brasz"""
        c4t = abs(c4t)
        tip_speed = n_rot*geom.r4
        return (0.6 * geom.clearance/geom.b4 * c4t/tip_speed
                * (4*pi/geom.b4/geom.n_blades*c4t*self.in2.c*cos(geom.alpha2/180*pi)/tip_speed**2
                   * (geom.r2s**2-geom.r2h**2)/((geom.r4-geom.r2s)*(1+tp4.D/self.in2.static.D)))**0.5 * tip_speed**2)

    def disc_friction_losses(self, geom: Geometry, tp4: ThermoProp, m: float, n_rot: float) -> float:
        """Calculates the impeller disc friction losses according to Daily & Nece"""
        if tp4.V < 0:
            print("Negative V", tp4)
        Re_y = 2. * n_rot * geom.r4**2 * tp4.D / tp4.V
        if Re_y > 3e5:
    ...
    def recirculation_losses(self, geom: Geometry, Df: float, alpha: float, n_rot: float) -> float:
    ...
    def calculate(self, geom: Geometry, op: OperatingCondition) -> None:
        ...
        try:
            rel3_temp = op.fld.thermo_prop('HS', self.in2.relative.H - dh_inc, self.in2.relative.S)
        except ThermoException:
            self.choke_flag = True
            return
        ...
        # Impeller Discharge
        h4_rel = 0.5 * ((geom.r4*op.n_rot)**2 - (geom.r2rms*op.n_rot)**2) + self.in2.relative.H
        try:
        ...
        def resolve_discharge_triangle(x: List[float]) -> List[float]:
            ...
            try:
                ...
                err.append((beta4_f_new - beta4_f) / 60.)
                ...
                # Correct pressure
                tp4_temp = op.fld.thermo_prop('HS', h4_rel - dh_losses_int, self.in2.relative.S)

                err.append((tp4_temp.P - tp4_r.P) / self.in2.relative.P + (abs(p4_rel - p4r)))
        ...
        # Guesses
        beta4_f0 = geom.beta4 - 10.
        A4_rel = 2*pi*geom.r4*geom.b4*geom.blockage[3]*cos(beta4_f0*pi/180)
        w4_guess = op.m / A4_rel / tp4_rel.D

        dh_df_guess = self.disc_friction_losses(geom, tp4_rel, op.m, op.n_rot)

        sol = optimize.root(resolve_discharge_triangle, x0=[beta4_f0, w4_guess, dh_df_guess, tp4_rel.P], tol=1e-4)
        if (sol.fun > 0.001).any():
            self.choke_flag = True
            return
        ...
        try:
            ...
            print(c4, w4)
            raise
        self.out.isentropic = op.fld.thermo_prop('PS', self.out.total.P, self.in2.static.S)
        ...
        self.losses.clearance = self.clearance_losses(geom, self.out.static, c4t, op.n_rot)

        self.losses.disc_friction = self.disc_friction_losses(geom, self.out.static, op.m, op.n_rot)
